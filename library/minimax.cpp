...
static constexpr int MIN_EVALUATION_VALUE = std::numeric_limits<int>::min();
static constexpr int MAX_EVALUATION_VALUE = std::numeric_limits<int>::max();

void MinimaxStandardGame::search(Position position, Piece piece, Move& result, std::atomic<bool>& running) {
    ...
    std::cout << "Depth: " << parameters.DEPTH << '\n';
...
int MinimaxStandardGame::minimax(unsigned int depth, unsigned int turns_from_root, int alpha, int beta, Piece type) {
    ...
    if (type == Piece::White) {
        ...
        for (const Move& move : moves) {
            make_move(move);
            const int evaluation = minimax(depth - 1, turns_from_root + 1, alpha, beta, Piece::Black);
            unmake_move(move);
        ...
        for (const Move& move : moves) {
            make_move(move);
            const int evaluation = minimax(depth - 1, turns_from_root + 1, alpha, beta, Piece::White);
            unmake_move(move);
...
void MinimaxStandardGame::random_move(Piece piece) {
    ...
    best_move = moves[0];  // TODO
}

std::vector<Move> MinimaxStandardGame::get_all_moves(Piece piece) {
...
void MinimaxStandardGame::get_moves_phase1(Piece piece, std::vector<Move>& moves) {
    assert(piece != Piece::None);
...
void MinimaxStandardGame::get_moves_phase2(Piece piece, std::vector<Move>& moves) {
    assert(piece != Piece::None);
...
void MinimaxStandardGame::get_moves_phase3(Piece piece, std::vector<Move>& moves) {
    assert(piece != Piece::None);
...
void MinimaxStandardGame::make_move(const Move& move) {
    switch (move.type) {
        case MoveType::Place:
            position[move.place_node_index] = move.piece;
            break;
        case MoveType::Move:
            position[move.move_destination_node_index] = move.piece;
            position[move.move_source_node_index] = Piece::None;
            break;
        case MoveType::PlaceTake:
            position[move.place_node_index] = move.piece;
            position[move.take_node_index] = Piece::None;
            break;
...
void MinimaxStandardGame::unmake_move(const Move& move) {
    switch (move.type) {
        case MoveType::Place:
            position[move.place_node_index] = Piece::None;
            break;
        case MoveType::Move:
            position[move.move_destination_node_index] = Piece::None;
            position[move.move_source_node_index] = move.piece;
            break;
        case MoveType::PlaceTake:
            position[move.place_node_index] = Piece::None;
            position[move.take_node_index] = opponent_piece(move.piece);
            break;
        case MoveType::MoveTake:
            position[move.move_destination_node_index] = Piece::None;
            position[move.move_source_node_index] = move.piece;
...
void MinimaxStandardGame::make_place_move(Piece piece, int place_node_index) {
    position[place_node_index] = piece;
}

void MinimaxStandardGame::unmake_place_move(int place_node_index) {
    position[place_node_index] = Piece::None;
}

void MinimaxStandardGame::make_move_move(Piece piece, int move_source_node_index, int move_destination_node_index) {
    position[move_destination_node_index] = piece;
    position[move_source_node_index] = Piece::None;
}

void MinimaxStandardGame::unmake_move_move(Piece piece, int move_source_node_index, int move_destination_node_index) {
    position[move_destination_node_index] = Piece::None;
    position[move_source_node_index] = piece;
}

int MinimaxStandardGame::evaluate_position() {  // TODO also evaluate piece positions
    positions_evaluated++;
...
unsigned int MinimaxStandardGame::calculate_material(Piece piece) {
    assert(piece != Piece::None);
...
unsigned int MinimaxStandardGame::calculate_freedom(Piece piece) {
    assert(piece != Piece::None);
...
unsigned int MinimaxStandardGame::calculate_piece_freedom(int index) {
...
bool MinimaxStandardGame::all_pieces_in_mills(Piece piece) {
    for (int i = 0; i < NODES; i++) {
        if (position[i] == piece) {
            if (!is_mill(piece, i)) {
                return false;
...
std::array<int, 5> MinimaxStandardGame::neighbor_free_positions(int index) {
...
Piece MinimaxStandardGame::opponent_piece(Piece type) {
    switch (type) {
        case Piece::White:
            return Piece::Black;
        case Piece::Black:
...
bool MinimaxStandardGame::is_mill(Piece piece, int index) {
    assert(piece != Piece::None);
...
bool MinimaxStandardGame::is_game_over() {
    const unsigned int total_white_pieces = total_number_of_pieces(Piece::White);
    ...
    if (calculate_freedom(Piece::White) == 0 && position.white_pieces_outside + position.black_pieces_outside == 0) {
        return true;
    }

    if (calculate_freedom(Piece::Black) == 0 && position.white_pieces_outside + position.black_pieces_outside == 0) {
        return true;
    }
...
unsigned int MinimaxStandardGame::number_of_pieces_outside(Piece type) {
    switch (type) {
        case Piece::White:
...
unsigned int MinimaxStandardGame::total_number_of_pieces(Piece type) {
    switch (type) {
        case Piece::White:
